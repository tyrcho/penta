...
class DocGeneratorTest extends munit.FunSuite:
  ...
  test("a chain-upgrade building's page states its tier, produces rate, and upgrade from/to") {
    ...
    generatedTo(tmp)
    val forest = read(tmp, EntityNames.vaultRoot(Lang.Fr) + "/" + EntityNames.buildingPath(BuildingKind.Forest, Lang.Fr))
  ...
  test("LaboFondamental's upgrade-to list has all 5 specific labs") {
    ...
    generatedTo(tmp)
    val fondamental =
      read(tmp, EntityNames.vaultRoot(Lang.Fr) + "/" + EntityNames.buildingPath(BuildingKind.LaboFondamental, Lang.Fr))
    List("Labo Naturel", "Labo Sombre", "Labo de Recherche", "Labo de la Loi", "Labo du Chaos").foreach { name =>
  ...
  test("a unit's page states its tier, plunder amount, spawning building link, and speed") {
    val tmp = Files.createTempDirectory("docgen-test")
    generatedTo(tmp)
    val goblin = read(tmp, EntityNames.vaultRoot(Lang.En) + "/" + EntityNames.unitPath(UnitKind.Goblin, Lang.En))
