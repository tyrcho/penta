...
class EloRatingTest extends munit.FunSuite:
  ...
  test("equal ratings: a win moves the winner up and the loser down by exactly half the K-factor") {
    val (a, b) =
      EloRating.updateRatings(EloRating.InitialRating, EloRating.InitialRating, scoreA = 1.0)
  ...
  test("equal ratings: a draw leaves both ratings unchanged") {
    val (a, b) =
      EloRating.updateRatings(EloRating.InitialRating, EloRating.InitialRating, scoreA = 0.5)
  ...
  test("a big favorite winning the expected result gains less than an even match would") {
    ...
    assert(
      favorite - 1800.0 < EloRating.KFactor / 2.0,
      s"expected gain under ${EloRating.KFactor / 2.0}, was ${favorite - 1800.0}"
    )
  }
  ...
  test("an underdog upset gains more than an even match would") {
    ...
    assert(
      underdog - 1200.0 > EloRating.KFactor / 2.0,
      s"expected gain over ${EloRating.KFactor / 2.0}, was ${underdog - 1200.0}"
    )
  }
