...
object Placement:
  ...
  def tryPlaceBuildingCached(
    ...
    tryPlaceBuildingKnowingReachability(
      ...
      col,
      row,
      blocksPath = !nonBlocking.contains(Pos(col, row))
    )
  ...
  def nonBlockingCells(state: MazeState): Set[Pos] =
    GridConfig.allCells.filterNot(cell => wouldBlockPath(state, cell.col, cell.row)).toSet
  ...
  private def tryPlaceBuildingKnowingReachability(
    ...
    val cost = effectiveCost(state, kind, kind.cost)
    for
      _ <- Either.cond(kind.buildableDirectly, (), PlacementError.CannotBuildDirectly)
      _ <- checkCellCheap(state, col, row)
      _ <- Either.cond(canAfford(state.resources, cost), (), PlacementError.InsufficientResources)
      _ <- checkMaxCount(state, kind)
  ...
  private[domain] def effectiveCost(
    ...
    if level <= 0 || kind.faction != Faction.Nature then baseCost
    else
  ...
  private def checkMaxCount(state: MazeState, kind: BuildingKind): Either[PlacementError, Unit] =
    kind.maxPerMaze match
      case Some(max) =>
  ...
  def tryUpgradeBuilding(
    ...
    for
      ...
      _ <- checkMaxCount(state, chosenKind)
      cost = effectiveCost(state, chosenKind, chosenKind.cost)
      _ <- Either.cond(canAfford(state.resources, cost), (), PlacementError.InsufficientResources)
  ...
  // No BuildingKind/ResearchSpec cost is ever supposed to list Gold itself as a cost (it's
  ...
  private def checkCellCheap(state: MazeState, col: Int, row: Int): Either[PlacementError, Unit] =
    ...
    else if Set(GridConfig.spawnCell, GridConfig.goalCell).contains(Pos(col, row)) then
      Left(PlacementError.OnSpawnOrGoal)
    else if state.buildingCells.contains(Pos(col, row)) then Left(PlacementError.CellOccupied)
    else Right(())

  private def wouldBlockPath(state: MazeState, col: Int, row: Int): Boolean =
    val blocked = state.buildingCells + Pos(col, row)
  ...
  private def placeBuilding(
    ...
    val building = Building(
      ...
      spawnCountdownMs = kind.spawns.map(_._2 / 2.0).getOrElse(0.0),
  ...
  private def upgradeBuilding(
    ...
    val spawnCountdownMs = nextKind.spawns.map(_._2 / 2.0).getOrElse(0.0)
