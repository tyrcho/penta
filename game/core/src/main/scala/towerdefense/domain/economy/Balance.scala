...
object Balance:
  ...
  // shape as the Necromancer (UnitKind.spawnFreezeMs), except the clone appears at the
  // *next* path cell rather than the summoner's own position (see UnitKind.
  ...
  // Shared per-unit plunder magnitude — Elf's real Wood theft (UnitKind.Elf.plunder)
  // uses it directly (1 unit of Wood per Elf that reaches the goal, clamped to what's
  // available); Goblin's Gold theft (UnitKind.Goblin.plunder) uses 2x this, preserving
  ...
  // shape (UnitKind.Minotaur.plunder), preserving Minotaure.md's original "Pille 10
  ...
  // Gold resource. A fraction of the cost of the BUILDING that produced the killed unit
  // (UnitKind.producedFrom/BuildingKind.cost — see CombatEngine.killGoldReward), not a flat
  // per-kill amount: a costlier producer (e.g. Labyrinth's Minotaur) naturally earns more
  // than a cheap one (Grove's Elf), without a separate "large unit" special case. Zero for
  // a creature summoned by another creature rather than built directly (a Soul, or a
  // self-cloned Tree — see Creature.summonedBy) — it cost nothing new to make, so killing
  // it earns nothing.
  val LoyalesKillGoldRewardFraction: Double = 0.05
  ...
  // cost of the BUILDING that made that unit (UnitKind.producedFrom), not a share
  ...
  // Soul, it stops advancing toward the goal for this long (see UnitKind.spawnFreezeMs/
