...
// side can build either Forest or Cave (see CLAUDE.md). Each Forest sends its Elf, and
...
object BattleEngine:
  ...
  private def maybeBuildThrottled(state: MazeState, cooldownMs: Double): (MazeState, Double) =
    if cooldownMs > 0 then (state, cooldownMs)
      ...
      val didBuild = built.forests.size > state.forests.size || built.caves.size > state.caves.size
