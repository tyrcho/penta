...
case class ComposedStrategy(
  ...
  def maybeBuild(state: MazeState, opponent: MazeState): MazeState =
    ...
    if candidates.isEmpty then state
      ...
      val layoutScores = candidates.map(c => layout.score(state, c.kind, Pos(c.col, c.row)))
  ...
  private def allCandidates(state: MazeState): Seq[Candidate] =
    ...
    for
      ...
      cell <- GridConfig.allCells
      result <- Placement
        .tryPlaceBuildingCached(state, kind, cell.col, cell.row, nonBlocking)
        .toOption
    yield Candidate(kind, cell.col, cell.row, result)
