...
// unreachable as long as any Wood income exists (see AiStrategyTest). Worth revisiting if
...
// BuildingKind.values exactly — the same invariant this file's own doc claims ("Both sides
// can build any directly-buildable BuildingKind") but that a hand-written literal list
// can't otherwise guarantee: WarCamp was added to the enum without a matching entry
