...
case class TickEvents(
    playerDeaths: List[Death] = Nil,
    aiDeaths: List[Death] = Nil,
    playerArrivals: List[UnitKind] = Nil,
    aiArrivals: List[UnitKind] = Nil,
    playerCorrupted: List[Corrosion] = Nil,
    aiCorrupted: List[Corrosion] = Nil
)

object TickEvents:
  val empty: TickEvents = TickEvents()
...
object BattleEngine:
  ...
  // ressources") lands in the attacker's own economy as the exact same resource drained
  // from the victim — real Wood for Elf, real Gold for Goblin/Minotaur (UnitKind.plunder,
  // a genuine transfer, not a conversion into some other currency). Deliberately NOT the
  // same amount as the victim's own loss (CombatEngine.moveCreatures' `stolen`, capped at
  // what they had): the attacker's credit here is always the full nominal plunder value,
  // even raiding a maze with nothing left to steal — "you always win res, even if the
  // opponent does not have them" (project owner's explicit request).
  private def creditPlunder(state: MazeState, plundered: Map[Resource, Double]): MazeState =
  ...
  private def spawnCreature(state: MazeState, kind: UnitKind): MazeState =
    val spawnPos = GridConfig.cellCenter(GridConfig.spawnCell)
    // A fresh Necromancer starts with a full Soul-summon countdown (kind.spawns' interval),
    // not 0 — it doesn't fire instantly the moment it's spawned. Inert (0.0) for every
    // other kind, which has no UnitKind.spawns at all.
    val initialCountdown = kind.spawns.map(_._2).getOrElse(0.0)
    ...
    val maxHp =
      if kind == UnitKind.Elf then
        ...
        kind.maxHp * (1.0 + allyElfCount * Balance.ElfHpBonusPerAlly)
      else kind.maxHp
    val creature =
      Creature(
        state.nextId,
