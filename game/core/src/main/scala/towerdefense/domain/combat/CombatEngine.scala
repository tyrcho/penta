...
// full nominal kind.plunder amount, even raiding a maze with nothing
// left to steal.
...
object CombatEngine:
  ...
  // Plunder varies by kind — see UnitKind.plunder: Elf takes wood only,
  ...
  private def moveCreatures(
    ...
    val angelCells =
      state.buildings.filter(_.kind == BuildingKind.Angel).map(b => Pos(b.col, b.row)).toSet
    val stasisCells =
      state.buildings.filter(_.kind == BuildingKind.StasisField).map(b => Pos(b.col, b.row)).toSet
    val (remaining, arrived) =
      state.creatures
        .map(stepCreature(_, state.creatures, blocked, angelCells, stasisCells, deltaMs))
        .partitionMap(identity)
    val plundered = arrived.flatMap(c => c.kind.plunder).groupMapReduce(_._1)(_._2)(_ + _)
  ...
  // Necromancien.md: "pendant 1 seconde, il reste immobile" (see UnitKind.
  ...
  private def advanceTowards(
    ...
    val target = GridConfig.cellCenter(nextCell)
  ...
  // *other* units, Wolf's own speed is already baked into UnitKind.speedPerMs). Multiple
  ...
  // Sourced from kind.dps (itself set from these same Balance constants per kind) rather
  // than re-matching on kind here, so the vault's generated "dps" frontmatter
  // (DocGenerator) can never drift from what actually deals damage in a match.
  private def auraDamagePerSecFor(kind: BuildingKind): Double = kind.dps
  ...
  // math is intentionally kept as kind-based special cases, not folded into BuildingKind/
  // UnitKind's own literal fields (see the refactor's confirmed scope).
  //
  ...
  private def watchtowerFiring(
    ...
    accumulateFiring(
      ticked,
      (w, acc) => accumulateWatchtowerHit(w, state.creatures, BuildingKind.Watchtower.dps, acc)
    )
  ...
  private def loyalesIntervalFor(state: MazeState, kind: BuildingKind): Double =
    ...
    if kind.faction == Faction.Loi then Balance.DamageTickIntervalMs / loyalesSpeedMultiplier
  ...
  // See Balance.LoyalesKillGoldRewardFraction's own doc: a fraction of the killed unit's
  // producing building's cost, 0 for a creature summoned by another creature (a Soul, or a
  // self-cloned Tree) rather than built directly.
  private def killGoldReward(c: Creature): Double =
    if c.summonedBy.isDefined then 0.0
    else
      c.kind.producedFrom.map(_.cost.values.sum).getOrElse(0.0) *
        Balance.LoyalesKillGoldRewardFraction
  ...
  // made that unit (UnitKind.producedFrom/BuildingKind.cost), not a share of the maze's
  // own stockpile and not the unit's own plunder value. Every unit kind has *some*
  // producing building with a nonzero cost (CreatureSpecsTest asserts this), so even a
  // non-plunderer death (Paladin, Wolf, Zombie, ...) now contributes something — unlike
    ...
    val (buildings, goldReward) =
      state.buildings.foldLeft((List.empty[Building], 0.0)) { case ((acc, reward), b) =>
        ...
        if b.kind != BuildingKind.PassingGate || b.constructionRemainingMs > 0.0 then
          ...
          val adjacent = Pathfinding.neighbors(Pos(b.col, b.row)).toSet
          ...
          if harvestedKinds.nonEmpty then
            val gateReward = harvestedKinds.map { kind =>
              val producingCost = kind.producedFrom.map(_.cost.values.sum).getOrElse(0.0)
              Balance.PassingGateHarvestFraction * producingCost
            }.sum
  ...
  private def accumulateAuraHits(
    ...
    val adjacent = Pathfinding.neighbors(Pos(forest.col, forest.row)).toSet
    creatures
  ...
  private def nearestTargetInRange(tower: Building, creatures: List[Creature]): Option[Long] =
    creatures
      .map(c => (c.id, chebyshevDistance(Pos(tower.col, tower.row), GridConfig.cellOf(c.pos))))
  ...
  private[domain] def chebyshevDistance(a: Pos, b: Pos): Int =
  ...
  // Zombie.md/Vampire.md/Ame.md's corruption rates (UnitKind.corruptionRatePerSec, 0.0 for
  // every kind without a corruption ability) — Corruption.md: "Les unites de cette faction
  ...
  private def applyCorruption(
    ...
    val corruptors = state.creatures.filter(_.kind.corruptionRatePerSec > 0.0)
    if corruptors.isEmpty then (state, Nil, Map.empty)
      ...
      val corruptionByCell = corruptors
        .groupMapReduce(c => GridConfig.cellOf(c.pos))(c =>
          c.kind.corruptionRatePerSec * sombresMultiplier * deltaMs / 1000.0
        )(
          _ + _
        )
      ...
      val buildingCellsBefore = state.buildings.map(b => Pos(b.col, b.row)).toSet
  ...
  private def splitDestroyed(buildings: List[Building]): (List[Corrosion], List[Building]) =
    ...
    (
      destroyed.map(b => Corrosion(b.id, b.kind, b.col, b.row, b.kind.cost)),
      remaining
    )

  private def applyCorruptionHits(
      buildings: List[Building],
      corruptionByCell: Map[Pos, Double]
  ): List[Building] =
    buildings.map { b =>
      val hits = Pathfinding.neighbors(Pos(b.col, b.row)).flatMap(corruptionByCell.get).sum
      if hits <= 0.0 then b
      else
  ...
  // Balance.GroveCorruptionHealPercentPerSec's doc): each tier heals at its own rate, via
  // BuildingKind.corruptionHealPercentPerSec (0.0 for every non-Nature kind).
  private def healBuildingCorruption(state: MazeState, deltaMs: Double): MazeState =
    val healers = state.buildings.filter(_.kind.corruptionHealPercentPerSec > 0.0)
    if healers.isEmpty || !state.buildings.exists(_.corruptionPercent > 0.0) then state
      ...
      state.copy(buildings = state.buildings.map { b =>
        if b.corruptionPercent <= 0.0 then b
          ...
          val heal = healers
            .filter(h => chebyshevDistance(Pos(h.col, h.row), Pos(b.col, b.row)) <= 1)
            .map(h => h.kind.corruptionHealPercentPerSec * deltaMs / 1000.0)
            .sum
          if heal <= 0.0 then b
  ...
  private def advanceCreatureSummons(state: MazeState, deltaMs: Double): MazeState =
    ...
    val summoners = state.creatures.flatMap(c => c.kind.spawns.map(spawn => (c, spawn)))
        ...
        summoners.foldLeft((List.empty[Creature], List.empty[Creature], state.nextId)) {
          case ((accUpdated, accNew, id), (summoner, (summonedKind, intervalMs))) =>
            ...
            if remaining > 0 then
              ...
              val summoned = summonChild(id, summoner, summonedKind, blocked)
              (
                summoner.copy(
                  spawnCountdownMs = remaining + intervalMs,
                  frozenMs = summoner.kind.spawnFreezeMs
                )
                  :: accUpdated,
                summoned :: accNew,
                id + 1
  ...
  private def summonChild(
    ...
    val spawnPos =
      if summoner.kind.spawnAtNextCell then nextPathCellCenter(summoner, blocked) else summoner.pos
    ...
    Creature(
      id,
      spawnPos,
      summonedKind.maxHp * childSizeFraction,
      summonedKind.maxHp * childSizeFraction,
      summonedKind.speedPerMs,
      summonedKind,
      spawnCountdownMs = summonedKind.spawns.map(_._2).getOrElse(0.0),
  ...
  // UnitKind.spawnAtNextCell) — falls back to the summoner's own position if it's
  // already at the goal or (shouldn't happen, placement guards this) has no route at all.
  private def nextPathCellCenter(summoner: Creature, blocked: Set[Pos]): Vec2 =
    val currentCell = GridConfig.cellOf(summoner.pos)
    if currentCell == GridConfig.goalCell then summoner.pos
    else
      Pathfinding.shortestPath(currentCell, GridConfig.goalCell, blocked) match
        case Some(path) if path.size > 1 => GridConfig.cellCenter(path(1))
        case _                           => summoner.pos
  ...
  def productionPerSec(state: MazeState, resource: Resource): Double =
    val base = state.buildings
      ...
      .groupBy(_.kind)
      .map { case (kind, bs) =>
        bs.size * kind.produces.getOrElse(resource, 0.0) *
  ...
  // Resource.engendreSource is None for Gold (and would be for any resource outside
  // Engendre.md's 5-cycle) — produceResources calls this for every Resource.values entry,
  // so that None short-circuits to 0.0 rather than needing a separate guard here.
  def engendreBoost(state: MazeState, resource: Resource): Double =
    resource.engendreSource match
      case None         => 0.0
      case Some(source) =>
        val sourceBuildingCount =
          state.buildings.count(b => b.kind.produces.getOrElse(source, 0.0) > 0.0)
  ...
  private def advanceSpawnTimers(
    ...
    def effectiveInterval(kind: BuildingKind, intervalMs: Double): Double =
      if kind.faction == Faction.Chaos then intervalMs * (1.0 - chaotiquesReduction)
      else intervalMs
