...
class LayoutPolicyTest extends munit.FunSuite:
  ...
  test("dangerScore combines path length with aura-damage exposure from existing forests") {
    ...
    val isolatedPath =
      Pathfinding
        .shortestPath(GridConfig.spawnCell, GridConfig.goalCell, noForests.buildingCells + ((6, 6)))
        .get
    assertEquals(
  ...
  test("dangerScore combines path length with ranged damage exposure from an existing watchtower") {
    ...
    val exposedPath =
      Pathfinding
        .shortestPath(GridConfig.spawnCell, GridConfig.goalCell, withTower.buildingCells + ((6, 6)))
        .get
    val inRangeCount = exposedPath.count(c =>
      CombatEngine.chebyshevDistance(c, (6, 5)) <= Balance.WatchtowerRangeCells
    )
    assertEquals(
  ...
  test(
    ...
    val path = Pathfinding
      .shortestPath(GridConfig.spawnCell, GridConfig.goalCell, state.buildingCells + ((6, 6)))
      .get
    val selfRangeCount =
      path.count(c => CombatEngine.chebyshevDistance(c, (6, 6)) <= Balance.WatchtowerRangeCells)
    assertEquals(
  ...
  test(
    ...
    val (col, row) = GridConfig.allCells
      ...
      .maxBy(c => FreeformLayout.dangerScore(state, c, isAuraCandidate = false))
    assert(
      FreeformLayout.score(state, BuildingKind.Watchtower, (col, row)) > FreeformLayout.score(
        state,
        BuildingKind.Grove,
        (col, row)
      )
  ...
  test(
    ...
    val built = Placement
      .tryPlaceBuilding(withResources(wood = 100.0), BuildingKind.Grove, col, row)
      .toOption
      .get
  ...
  test("ForcedOpeningLayout scores the opening's kind at position 0, before any buildings exist") {
    ...
    assertEquals(
      layout.score(MazeState.initial, BuildingKind.Cave, (5, 5)),
      FreeformLayout.score(MazeState.initial, BuildingKind.Cave, (5, 5))
    )
  }
  ...
  test("ForcedOpeningLayout vetoes every other kind while an opening position is still pending") {
    val layout = ForcedOpeningLayout(Seq(BuildingKind.Cave, BuildingKind.WarCamp), FreeformLayout)
    assertEquals(
      layout.score(MazeState.initial, BuildingKind.Grove, (5, 5)),
      Double.NegativeInfinity
    )
    assertEquals(
      layout.score(MazeState.initial, BuildingKind.WarCamp, (5, 5)),
      Double.NegativeInfinity
    )
  }
  ...
  test("ForcedOpeningLayout advances to the next opening kind once the previous one exists") {
    ...
    assertEquals(
      layout.score(oneCave, BuildingKind.WarCamp, (5, 5)),
      FreeformLayout.score(oneCave, BuildingKind.WarCamp, (5, 5))
    )
    assertEquals(layout.score(oneCave, BuildingKind.Cave, (5, 5)), Double.NegativeInfinity)
  }
  ...
  test("ForcedOpeningLayout defers entirely to inner once the opening is complete") {
    ...
    assertEquals(
      layout.score(pastOpening, BuildingKind.Watchtower, (5, 5)),
      FreeformLayout.score(pastOpening, BuildingKind.Watchtower, (5, 5))
    )
    assertEquals(
      layout.score(pastOpening, BuildingKind.Grove, (5, 5)),
      FreeformLayout.score(pastOpening, BuildingKind.Grove, (5, 5)),
  ...
  test("HealClusterLayout adds a per-neighbor bonus on top of inner's score for a healer kind") {
    ...
    assertEquals(layout.score(noNeighbors, BuildingKind.Grove, (5, 5)), 0.0)
    assertEquals(layout.score(oneNeighbor, BuildingKind.Grove, (5, 5)), 5.0)
    assertEquals(layout.score(twoNeighbors, BuildingKind.Grove, (5, 5)), 10.0)
  }
  ...
  test("HealClusterLayout only counts buildings within Chebyshev distance 1, not farther ones") {
    ...
    assertEquals(layout.score(farAway, BuildingKind.Grove, (5, 5)), 0.0)
  }
  ...
  test("HealClusterLayout leaves non-healer kinds entirely to inner, no bonus") {
    ...
    assertEquals(
      layout.score(state, BuildingKind.Cave, (5, 5)),
      FreeformLayout.score(state, BuildingKind.Cave, (5, 5))
    )
  }
  ...
  test("CountCapLayout vetoes the capped kind once countsAs buildings reach the cap") {
    ...
    assertEquals(
      layout.score(oneLab, BuildingKind.LaboFondamental, (6, 6)),
      Double.NegativeInfinity
    )
  }
  ...
  test("CountCapLayout defers to inner below the cap") {
    ...
    assertEquals(
      layout.score(MazeState.initial, BuildingKind.LaboFondamental, (6, 6)),
      FreeformLayout.score(MazeState.initial, BuildingKind.LaboFondamental, (6, 6))
    )
  }
  ...
  test(
    ...
    assertEquals(
      layout.score(upgraded, BuildingKind.LaboFondamental, (6, 6)),
      Double.NegativeInfinity,
  ...
  test("CountCapLayout leaves every other kind entirely to inner, no veto") {
    ...
    val oneLab =
      MazeState.initial.copy(buildings = List(building(1, 5, 5, BuildingKind.LaboFondamental)))
    assertEquals(
      layout.score(oneLab, BuildingKind.Watchtower, (6, 6)),
      FreeformLayout.score(oneLab, BuildingKind.Watchtower, (6, 6))
    )
  }
