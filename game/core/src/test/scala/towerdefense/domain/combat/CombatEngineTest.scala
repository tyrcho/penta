...
class CombatEngineTest extends munit.FunSuite:
  ...
  test("enemy reaching the goal cell is removed") {
    val goalPos = GridConfig.cellCenter(GridConfig.goalCell)
    val creature =
  ...
  test("enemy re-routes around a forest blocking its straight-line step") {
    ...
    val result = CombatEngine.tick(state, deltaMs = 1.0)
    assertEquals(GridConfig.cellOf(result.state.creatures.head.pos), Pos(0, 1))
  }
  ...
  test(
    ...
    val gate = Building(100, col = 5, row = 5, BuildingKind.PassingGate, 0.0)
    // A Zombie specifically: UnitKind.Zombie.plunder is empty (Zombie.md gives it
    // no plunder ability — its value is corrupting buildings instead), so this proves the
    // harvest is scored off Tomb's cost (UnitKind.Zombie.producedFrom), not the
  ...
  test("a passing gate does not harvest gold (or flash) for a death far from it") {
    ...
    val result = CombatEngine.tick(state, deltaMs = 1000.0)
    assertEquals(result.state.creatures, Nil)
    // The Watchtower kill itself grants Gold (a Loi mechanic, unrelated to Passing Gate) —
    // this test is only about the FAR gate's own contribution being exactly 0, not about
    // total Gold staying at 0.
    val expectedGold =
      Balance.GroveCostWood * Balance.LoyalesKillGoldRewardFraction // Elf's producing building
    assertEqualsDouble(
  ...
  test("a building corrupted to 100% is destroyed and reported for refund") {
    ...
    assertEquals(
      result.corrupted,
      List(Corrosion(100, BuildingKind.Grove, 5, 5, BuildingKind.Grove.cost))
    )
  }
  ...
  test(
    ...
    assertEquals(UnitKind.Necromancer.spawns, Some((UnitKind.Soul, Balance.SoulSummonIntervalMs)))
  }
  ...
  // clone appears one cell further along the summoner's own path — see UnitKind.
  ...
  test(
    ...
    val expectedNextCell =
      Pathfinding.shortestPath(Pos(0, 0), GridConfig.goalCell, Set.empty).get(1)
    assertEquals(clone.pos, GridConfig.cellCenter(expectedNextCell))
  ...
  test(
    "a watchtower kill earns its own maze Gold proportional to the victim's producing building"
  ) {
    val watchtower = Building(100, col = 5, row = 5, BuildingKind.Watchtower, 0.0)
    val target = Creature(
      1,
    ...
    assertEqualsDouble(
      result.state.resources.getOrElse(Resource.Gold, 0.0),
      Balance.GroveCostWood * Balance.LoyalesKillGoldRewardFraction,
      1e-9
    )
  }

  test("an Angel kill earns its own maze Gold too, not just Watchtower") {
    val angel = Building(100, col = 5, row = 5, BuildingKind.Angel, 0.0)
    val target = Creature(
      1,
      GridConfig.cellCenter(6, 5),
  ...
  test("a watchtower kill on a Minotaur earns Gold off Labyrinth's cost, not a flat amount") {
    val watchtower = Building(100, col = 5, row = 5, BuildingKind.Watchtower, 0.0)
    val target =
      Creature(
        1,
    ...
    val result = CombatEngine.tick(state, deltaMs = 1000.0)
    assertEquals(result.state.creatures, Nil)
    val labyrinthCost = Balance.LabyrintheCostWood + Balance.LabyrintheCostFire
    assertEqualsDouble(
      result.state.resources.getOrElse(Resource.Gold, 0.0),
      labyrinthCost * Balance.LoyalesKillGoldRewardFraction,
      1e-9
    )
  }

  test("a watchtower kill on a summoned Soul earns no Gold — it cost nothing new to make") {
    val watchtower = Building(100, col = 5, row = 5, BuildingKind.Watchtower, 0.0)
    val soul = Creature(
      1,
      GridConfig.cellCenter(6, 5),
      hp = 1.0,
      maxHp = Balance.SoulMaxHp,
      speedPerMs = 0.0,
      UnitKind.Soul,
      summonedBy = Some(2)
    )
    val state = withResources().copy(creatures = List(soul), buildings = List(watchtower))
    val result = CombatEngine.tick(state, deltaMs = 1000.0)
    assertEquals(result.state.creatures, Nil)
  ...
  test(
    "a watchtower kill on a self-cloned Tree earns no Gold, but an original Tree still does"
  ) {
    val watchtower = Building(100, col = 5, row = 5, BuildingKind.Watchtower, 0.0)
    val clonedTree = Creature(
      1,
      GridConfig.cellCenter(6, 5),
      hp = 1.0,
      maxHp = Balance.TreeMaxHp,
      speedPerMs = 0.0,
      UnitKind.Tree,
      summonedBy = Some(2)
    )
    val clonedState =
      withResources().copy(creatures = List(clonedTree), buildings = List(watchtower))
    val clonedResult = CombatEngine.tick(clonedState, deltaMs = 1000.0)
    assertEquals(clonedResult.state.creatures, Nil)
    assertEquals(clonedResult.state.resources.getOrElse(Resource.Gold, 0.0), 0.0)

    val originalTree = clonedTree.copy(summonedBy = None)
    val originalState =
      withResources().copy(creatures = List(originalTree), buildings = List(watchtower))
    val originalResult = CombatEngine.tick(originalState, deltaMs = 1000.0)
    assertEquals(originalResult.state.creatures, Nil)
    assertEqualsDouble(
      originalResult.state.resources.getOrElse(Resource.Gold, 0.0),
      Balance.StonehengeCostWood * Balance.LoyalesKillGoldRewardFraction,
      1e-9
    )
  }
