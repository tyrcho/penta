...
class DemolitionTest extends munit.FunSuite:
  ...
  test("destroys a cave and refunds half its wood and fire cost") {
    val (col, row) = (5, 5)
    val withCave =
      Placement.tryPlaceBuilding(MazeState.initial, BuildingKind.Cave, col, row).toOption.get
  ...
  test("destroys a labyrinthe and refunds half its wood and fire cost") {
    val (col, row) = (5, 5)
    val rich =
      MazeState.initial.copy(resources = Map(Resource.Wood -> 1_000.0, Resource.Fire -> 1_000.0))
    val withLabyrinthe =
      Placement.tryPlaceBuilding(rich, BuildingKind.Labyrinth, col, row).toOption.get
    ...
    assertEquals(
      result.resources(Resource.Wood),
      withLabyrinthe.resources(
        Resource.Wood
      ) + Balance.LabyrintheCostWood * Balance.DemolishRefundFraction
    )
    assertEquals(
      result.resources(Resource.Fire),
      withLabyrinthe.resources(
        Resource.Fire
      ) + Balance.LabyrintheCostFire * Balance.DemolishRefundFraction
    )
  }
  ...
  test("destroys an eglise and refunds half its wood and light cost") {
    val (col, row) = (5, 5)
    val rich =
      MazeState.initial.copy(resources = Map(Resource.Wood -> 1_000.0, Resource.Light -> 1_000.0))
    val withEglise =
    ...
    assertEquals(
      ...
      withEglise.resources(
        Resource.Light
      ) + Balance.EgliseCostLight * Balance.DemolishRefundFraction
    )
  }
  ...
  test("destroys a watchtower and refunds half its wood and light cost") {
    ...
    assertEquals(
      result.resources(Resource.Wood),
      withWatchtower.resources(
        Resource.Wood
      ) + Balance.WatchtowerCostWood * Balance.DemolishRefundFraction
    )
    assertEquals(
      result.resources(Resource.Light),
      withWatchtower.resources(
        Resource.Light
      ) + Balance.WatchtowerCostLight * Balance.DemolishRefundFraction
    )
  }
  ...
  test("destroys a tomb and refunds half its wood and shadow cost") {
    val (col, row) = (5, 5)
    val withTomb =
      Placement.tryPlaceBuilding(MazeState.initial, BuildingKind.Tomb, col, row).toOption.get
  ...
  test(
    "destroys a Science lab and refunds half its upgrade cost, freeing up the max-one-per-kind slot"
  ) {
    val (col, row) = (5, 5)
    val rich =
      MazeState.initial.copy(resources = Map(Resource.Wood -> 1_000.0, Resource.Crystal -> 1_000.0))
    val withBase =
      Placement.tryPlaceBuilding(rich, BuildingKind.LaboFondamental, col, row).toOption.get
    val withLab =
    ...
    assertEquals(
      ...
      withLab.resources(
        Resource.Crystal
      ) + Balance.LaboNaturelCostCrystal * Balance.DemolishRefundFraction
    )
    // Rebuilding after demolishing is allowed — maxPerMaze counts current buildings only.
    val withNewBase =
      Placement.tryPlaceBuilding(result, BuildingKind.LaboFondamental, col, row).toOption.get
    assertEquals(
  ...
  test(
    "destroying the only wall of a corridor is allowed (removing an obstacle can't seal a path)"
  ) {
    ...
    val withWall = corridor.foldLeft(rich) { (state, cell) =>
      Placement
        .tryPlaceBuilding(state, BuildingKind.Grove, cell._1, cell._2)
        .toOption
        .getOrElse(state)
    }
